/**
* The `Matter.MyRender` module is a simple canvas based renderer for visualising instances of `Matter.Engine`.
...
(function() {
    ...
    MyRender.create = function(options) {
        ...
        render.controller = MyRender;
        render.options.showBroadphase = false;

        if (render.options.pixelRatio !== 1) {
            MyRender.setPixelRatio(render, render.options.pixelRatio);
        }
    ...
    MyRender.run = function(render) {
        (function loop(time){
            ...
            if (render.options.showStats || render.options.showDebug) {
                MyRender.stats(render, render.context, time);
            }

            if (render.options.showPerformance || render.options.showDebug) {
                MyRender.performance(render, render.context, time);
            }
        })();
    };

    /**
     * Ends execution of `MyRender.run` on the given `render`, by canceling the animation frame request event loop.
     * @method stop
     * @param {render} render
     */
     ...
     * 
     * Follow this call with `MyRender.lookAt` if you need to change the render bounds.
     * 
     * See also `MyRender.setPixelRatio`.
     * @method setSize
     * @param {render} render
    ...
    MyRender.world = function(render, time) {
        ...
        if (options.hasBounds) {
            ...
            // transform the view
            MyRender.startViewTransform(render);
        ...
        if (!options.wireframes || (engine.enableSleeping && options.showSleeping)) {
            ...
            MyRender.bodyWireframes(render, bodies, context);
        }

        if (options.showBounds)
            MyRender.bodyBounds(render, bodies, context);

        if (options.showAxes || options.showAngleIndicator)
            MyRender.bodyAxes(render, bodies, context);

        if (options.showPositions)
            MyRender.bodyPositions(render, bodies, context);

        if (options.showVelocity)
            MyRender.bodyVelocity(render, bodies, context);

        if (options.showIds)
            MyRender.bodyIds(render, bodies, context);

        if (options.showSeparations)
            MyRender.separations(render, engine.pairs.list, context);

        if (options.showCollisions)
            MyRender.collisions(render, engine.pairs.list, context);

        if (options.showVertexNumbers)
            MyRender.vertexNumbers(render, bodies, context);

        if (options.showMousePosition)
            MyRender.mousePosition(render, render.mouse, context);

        MyRender.constraints(constraints, context);

        if (options.hasBounds) {
            // revert view transforms
            MyRender.endViewTransform(render);
        }
    ...
    var _updateTiming = function(render, time) {
        ...
        timing.delta = time - timing.lastTime || MyRender._goodDelta;
    ...
    */
    ...
    /**
     * A back-reference to the `Matter.MyRender` module.
     *
     * @deprecated
     * @property controller
     * @type render
